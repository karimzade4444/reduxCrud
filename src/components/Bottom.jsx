...
const Bottom = () => {
  ...
  return (
    <div className="grid grid-cols-3 items-center place-items-center gap-6 p-5">
      {data?.map((el) => {
        return (
          <div key={el.id} className="w-100 h-80 bg-black/80 rounded-xl">
            ...
            <div className="flex justify-between items-center pl-5 pr-5 pt-2">
              ...
              <button
                className="text-white w-25 h-8 border-2 border-gray-400 rounded-xl cursor-pointer"
                onClick={() => dispatch(openViewModal(el))}
              >
                Просмотр
              </button>

              <button
                className="text-white w-25 h-8 bg-red-800 rounded-xl cursor-pointer"
                onClick={() => dispatch(deleteBlock(el.id))}
              >
                Удалить
              </button>
            </div>
